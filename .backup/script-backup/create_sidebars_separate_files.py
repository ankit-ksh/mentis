...
def generate_sidebar_files(json_file):
    try:
    ...
    for level, courses in level_courses.items():
        for course in courses:
            ...
            # Add preface information (About the course, Important info, etc.)
            sidebar_content.append({
                'text': course['title'],
                'items': [
                    { 'text': 'About the course', 'link': f'/{level}/{course_acronym}/about' },
                    { 'text': 'Important information and links', 'link': f'/{level}/{course_acronym}/info' },
                    { 'text': 'Reference', 'link': f'/{level}/{course_acronym}/reference' }
                ]
            })
            ...
            for week in range(1, num_weeks + 1):
                weeks_content.append({
                    ...
                    'collapsed': True,
                    'items': [
                        { 'text': 'Content', 'link': f'/{level}/{course_acronym}/week{week}/content' },
                        { 'text': 'Questions', 'link': f'/{level}/{course_acronym}/week{week}/questions' }
                    ]
                })
            ...
            # Write this course's sidebar to a separate file
            sidebar_file = os.path.join(output_dir, f'{level}_{course_acronym}.mts')
...
if len(sys.argv) < 2:
    print("Usage: python generate_sidebar_files.py <courses.json>")
    sys.exit(1)

# Get the input JSON file from the command-line argument
json_file = sys.argv[1]

# Generate sidebar files from the provided JSON file
generate_sidebar_files(json_file)
